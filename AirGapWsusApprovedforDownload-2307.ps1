...
Function Approve-Windows10
{
    ...
    {
        if ($update.IsSuperseded -ne 'True' -and $update.ProductTitles -like 'Windows 10, version 1903 and later' -and $update.Title -notlike '*ARM*' -and $update.Title -like '*2*H2*' -and $update.Title -notlike '*x86-based*' -and $update.UpdateClassificationTitle -notlike 'Upgrades')
        {
...
Function Approve-Windows11
{
    foreach ($update in $updates)
    {
        if ($update.IsSuperseded -ne 'True' -and $update.ProductTitles -like 'Windows 11' -and $update.Title -and $update.Title -like '*2*H2*' -notlike '*ARM*' -and $update.Title -notlike '*x86-based*' -and $update.UpdateClassificationTitle -notlike 'Upgrades')
        {
          $update[0].Approve(“Install”,$Group)
            #write-output "Approved Update:" $update.Title 
            $script:countapprove++
